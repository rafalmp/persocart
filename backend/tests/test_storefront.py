...
def store(db: None):  # type: ignore[no-untyped-def]
    ...
    mild = CategoryFilterOption.objects.create(
        filter=heat_f, label="Mild", value="mild", position=0
    )
    hot = CategoryFilterOption.objects.create(
    ...
    vegan = CategoryFilterOption.objects.create(
        filter=tags_f, label="Vegan", value="vegan", position=0
    )
    spicy = CategoryFilterOption.objects.create(
        filter=tags_f, label="Spicy", value="spicy", position=1
    )
    ...
    p3 = Product.objects.create(
        category=sub, name="Dead Sauce", price=Decimal("9.00"), is_active=False
    )
...
class TestStorefrontCategories:
    ...
    def test_public_tree(self, api_client: APIClient, store: dict) -> None:
        ...
        root_node = next(n for n in resp.data if n["slug"] == "food")
...
class TestStorefrontProducts:
    ...
    def test_single_choice_filter(self, api_client: APIClient, store: dict) -> None:
        ...
        assert resp.data["results"][0]["name"] == "Mild Sauce"
    ...
    def test_number_range_filter(self, api_client: APIClient, store: dict) -> None:
        ...
        assert resp.status_code == 200
        assert resp.data["count"] == 1
        assert resp.data["results"][0]["name"] == "Hot Sauce"
    ...
    def test_products_include_descendants(self, api_client: APIClient, store: dict) -> None:
        ...
        assert resp.status_code == 200
        assert resp.data["count"] == 2  # p1 and p2 from sub
...
class TestStorefrontProductDetail:
    ...
    def test_product_detail(self, api_client: APIClient, store: dict) -> None:
        ...
        assert resp.status_code == 200
        assert resp.data["name"] == "Mild Sauce"
        assert len(resp.data["filterValues"]) >= 1
